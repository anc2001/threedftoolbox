...
class CameraRelativePosition:
    ...
    def get_camera_position(self, offset_3d=np.array([0, 0, 0])):
        ...
        self.camera_position += (
            offset_3d[0] * self.vt_3d
            + offset_3d[1] * self.z_axis
            + offset_3d[2] * self.direction_3d
        )
...
class FloorLine:
    def __init__(self, start_pt=np.array([0, 0]), end_pt=np.array([0, 0])):
        ...
        self.center_pt = (self.start_pt + self.end_pt) / 2.0
        self.len = np.linalg.norm(self.start_end_vt)
        self.vt = (
            self.start_end_vt / self.len
            if self.len > dist_epsilon
            else np.array([0, 0])
        )
...
class BoundingBox2d:
    ...
    def update_corner_pts(self):
        self.corner_pts = np.array(
            [
                self.min,
                np.array([self.max[0], self.min[1]]),
                self.max,
                np.array([self.min[0], self.max[1]]),
                self.min,
            ]
        )
...
class InstanceBox:
    def __init__(
        self,
        center_pt_2d=np.array([0, 0]),
        direction_2d=np.array([0, 0]),
        width=0,
        depth=0,
        height=0.0,
        type="",
    ...
    def update_corner_pts(self):
        self.corner_pts = np.array(
            [
                self.center_pt
                - self.vt * self.width / 2.0
                - self.direction * self.depth / 2.0,
                self.center_pt
                + self.vt * self.width / 2.0
                - self.direction * self.depth / 2.0,
                self.center_pt
        ...
        self.box.set_min_max(
            min_pt=np.array([min(self.corner_pts[:, 0]), min(self.corner_pts[:, 1])]),
...
class RoomFloor:
    def __init__(self, floor_info=[], window_info=[]):
        ...
        self.floor_lines = [
            FloorLine(self.floor_vertexes[i], self.floor_vertexes[i + 1])
    ...
    def get_nearest_wall_index(self, pt=np.array([0, 0])):
        para_dist_list = [
            get_nearest_para([floor_line.start_pt, floor_line.end_pt], pt)
...
class ChildrenRoom(RoomFloor):
    ...
    def calc_key_lines(self):
        window_center_pt = sum(self.window_pts) / 2.0

        floor_line_count = len(self.floor_lines)
        for line_index, line in enumerate(self.floor_lines):
            pt_para, pt_dist = get_nearest_para(
                [line.start_pt, line.end_pt], window_center_pt
            )
            if (
                -dist_epsilon < pt_para < line.len + dist_epsilon
                and pt_dist < dist_epsilon
            ):
                prev_line = self.floor_lines[
                    (line_index - 1 + floor_line_count) % floor_line_count
                ]
                next_line = self.floor_lines[(line_index + 1) % floor_line_count]

                if pt_para < line.len / 2.0:
                    self.key_lines = [
                        {"line": next_line, "window_side": "none"},
                        {"line": prev_line, "window_side": "next"},
                    ]
                else:
                    self.key_lines = [
                        {"line": prev_line, "window_side": "none"},
