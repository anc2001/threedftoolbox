    ...
    def generate_floor(self, floor_mesh_list, room_id):
        ...
        for floor_mesh in floor_mesh_list:
            ...
            for i in range(len(floor_mesh_xyz) // 3):
                xz_list.append(
                    [float(floor_mesh_xyz[3 * i]), float(floor_mesh_xyz[3 * i + 2])]
                )
        ...
        for i in range(len(xz_faces_list) // 3):
            ...
            if (
                xz_faces_list[3 * i][0] == xz_faces_list[3 * i + 1][0]
                and xz_faces_list[3 * i][0] == xz_faces_list[3 * i + 2][0]
            ) or (
                xz_faces_list[3 * i][1] == xz_faces_list[3 * i + 1][1]
                and xz_faces_list[3 * i][1] == xz_faces_list[3 * i + 2][1]
            ):
                continue

            grid_line_list = self.tool.grid_to_multi_line(
                xz_faces_list[3 * i], xz_faces_list[3 * i + 1], xz_faces_list[3 * i + 2]
            )

            if (
                grid_line_list[0][0] == grid_line_list[0][1]
                or grid_line_list[1][0] == grid_line_list[1][1]
                or grid_line_list[2][0] == grid_line_list[2][1]
            ):
                continue
        ...
        if self.floor_tool.connect_line_clockwise(once_line_list):
            if len(self.floor_tool.clockwise_line_list) == 1:
                ...
                self.clockwise_line_list = self.floor_tool.clockwise_line_list[
                    idx_len_list.index(max(idx_len_list))
                ]
        else:
            logger.e(
                "%s cannot generate floor: %d"
                % (room_id, len(self.clockwise_line_list))
            )
            return False

        if self.tool.is_closed(self.clockwise_line_list):
            self.clockwise_line_list = self.tool.merge_line(self.clockwise_line_list)
            self.clockwise_line_list = self.tool.overlap_line_deduplication(
                self.clockwise_line_list
            )
        else:
            return False
