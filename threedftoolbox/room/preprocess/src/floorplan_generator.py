...
class FloorplanGenerator(object):
    ...
    def generate_room_info(self, room):
        ...
        try:
            ...
            for children in room_children_list:
                for (
                    mesh_uid,
                    mesh_value,
                ) in self.json_information_acquirer.mesh_dict.items():
                    if (
                        mesh_value["type"] in self.mesh_type_list
                        and children in mesh_uid
                    ):
            ...
            logger.e("%s does not have the required key: %s" % (room["instanceid"], e))
            return False
        ...
        if self.floor_generator.generate_floor(floor_mesh_list, room["instanceid"]):
            ...
            if len(door_mesh_list) > 0:
                ...
                if self.door_generator.generate_door(door_mesh_list, line_list):
                    ...
                    room_dict["door_height_limit"] = (
                        self.door_generator.height_limit_list
                    )
            ...
            if len(hole_mesh_list) > 0:
                ...
                if self.door_generator.generate_door(hole_mesh_list, line_list):
        ...
        if self.door_generator.generate_door(window_mesh_list, line_list):
            ...
            room_dict["window_height_limit"] = self.door_generator.height_limit_list
        else:
            room_dict["window"] = []
        ...
        if self.baywindow_generator.generate_baywindow(
            ...
            room_dict["baywindow_height"] = self.baywindow_generator.baywindow_height
            room_dict["baywindow_height_limit"] = (
                self.baywindow_generator.baywindow_height_limit
            )
        else:
            room_dict["baywindow"] = []
