...
def get_floor_info(filename):
    ...
    for room_info in room_floors["floorplan"]:
        ...
        for i in range(0, len(room_info["floor"]), 2):
            floor_info.append([room_info["floor"][i], room_info["floor"][i + 1]])
        ...
        floor_dict[room_id] = {"floor": floor_info, "window": window_info}
...
def get_camera_visible_poly(camera_info, floor_info):
    ...
    k1 = (
        np.cos(np.deg2rad(half_fov)) * c_unit_dir
        + np.sin(np.deg2rad(half_fov)) * c_unit_normal
    )
    k2 = (
        np.cos(np.deg2rad(half_fov)) * c_unit_dir
        - np.sin(np.deg2rad(half_fov)) * c_unit_normal
    )
    camera_insight_poly = sg.Polygon(
        [camera_pos, camera_pos + 1e3 * k1, camera_pos + 1e3 * k2, camera_pos]
    )
