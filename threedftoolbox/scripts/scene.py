import json
import numpy as np
from threedftoolbox.scripts.utils import (
    read_obj,
    save_obj,
    read_mesh_attr,
    transform_v,
...
class Scene:
    ...
    def output_with_room_id(
        ...
        if not os.path.exists(savepath):
            os.mkdir(savepath)
        self.house.output_with_room_id(
            savepath + "/" + self.house.uid,
            room_id,
            center,
            select_mesh_type,
...
class House:
    ...
    def output(
        self, savepath, select_room_type, select_mesh_type, select_furniture_type
    ):
        if not os.path.exists(savepath):
            os.mkdir(savepath)
    ...
    def output_with_room_id(
        self, savepath, room_id, center, select_mesh_type, select_furniture_type
    ):
        if not os.path.exists(savepath):
            os.mkdir(savepath)
        self.rooms[room_id].output(
            savepath, center, [], select_mesh_type, select_furniture_type
        )
...
class Room:
    ...
    def output_meshes(self, savepath, select_mesh_type):
        ...
        for constructid, all_mesh in self.meshes.items():
            ...
            for mesh in all_mesh:
                ...
                try:
                    output.append(
                        [mesh.instanceid, v - self.center, faces, vt, texture]
                    )
                    all_output.append(
                        [mesh.instanceid, v - self.center, faces, vt, texture]
                    )
    ...
    def output_furniture(self, savepath, select_furniture_type):
        ...
        for instance in self.furniture:
            ...
            obj_path = (
                Config.FUTURE_PATH + "/" + instance.info.jid + "/raw_model_tri.obj"
            )
            v = read_obj(obj_path)
            ...
            save_obj(
                ...
                os.path.join(
                    savepath,
                    instance.info.super_category.replace("/", "_")
                    + "_"
                    + instance.info.jid
                    + "_"
                    + str(number)
    ...
    if "material" in data:
        for mat in data["material"]:
            m = Material(
                ...
                mat["jid"],
                (
                    np.reshape(mat["UVTransform"], [3, 3])
                    if "UVTransform" in mat
                    else None
                ),
                mat["texture"],
            )
