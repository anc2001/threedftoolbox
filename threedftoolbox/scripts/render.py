...
parser.add_argument(
    "--scene_path", type=str, default="./scenes", help="the path of scene obj files"
)
parser.add_argument(
    "--json_path", type=str, default="./3D-FRONT", help="the path of scene json files"
)
parser.add_argument("--res", type=float, default=512, help="render image resolution.")
...
def render_function(model_list, texture_file, cam_info, save_path):
    ...
    for info in cam_info:
        ...
        depth_file_output.file_slots[0].path = os.path.join(
            save_path, "depth_%02d" % (idx)
        )
        normal_file_output.file_slots[0].path = os.path.join(
            save_path, "normal_%02d" % (idx)
        )
...
for scene in scene_list:
    ...
    for room in room_list:
        ...
        for obj_name in os.listdir(os.path.join(args.scene_path, scene, room)):
            ...
            tex_list.append(
                os.path.join(
                    args.scene_path, scene, room, obj_name.replace("obj", "png")
                )
            )
