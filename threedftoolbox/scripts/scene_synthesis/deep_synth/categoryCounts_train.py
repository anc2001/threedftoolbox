...
model = DiscreteNADEModule(
    data_size=data_size, data_domain_sizes=data_domain_sizes, hidden_size=data_size
)
...
optimizer = optim.Adam(
    model.parameters(), lr=learning_rate, betas=(0.9, 0.999), eps=1e-6
)
    ...
    for batch_idx, data in enumerate(train_loader):
        ...
        if num_seen >= 10000:
            ...
            if current_epoch % 5 == 0:
                torch.save(
                    model.state_dict(),
                    f"{save_dir}/categoryCounts_epoch_{current_epoch}.pt",
                )
                torch.save(
                    optimizer.state_dict(), f"{save_dir}/categoryCounts_optim_backup.pt"
                )
