...
class Simulator:
    def __init__(
        ...
        if data_dir_base:
            ...
            self._data_dir_base = os.path.join(data_root_dir, "3d_front")
    ...
    def add_room(self, node, wall=True, floor=True, ceiling=False):
        ...
        if wall:
            # print('---------------------------')
            # print(f'{basename}w.obj')
            # print('---------------------------')
            body_wall = add_architecture(
                node, f"{basename}w.obj", ""
        ...
        for node in house.nodes:
            ...
            if node.type == "Room":
                ceil = (
                    False
                    if no_ceil
                    else not (hasattr(node, "hideCeiling") and node.hideCeiling == 1)
                )
                wall = (
                    False
                    if (no_walls or use_separate_walls)
                    else not (hasattr(node, "hideWalls") and node.hideWalls == 1)
                )
                floor = (
                    False
                    if no_floor
                    else not (hasattr(node, "hideFloor") and node.hideFloor == 1)
                )
                node.body = self.add_room(node, wall=wall, floor=floor, ceiling=ceil)
            if node.type == "Box":
                half_widths = list(map(lambda x: 0.5 * x, node.dimensions))
                node.body = self.add_box(
                    obj_id=node.id,
    ...
    def set_state(self, obj_id, position, rotation_q):
        body = self._obj_id_to_body[obj_id]
        rot_q = np.roll(
            rotation_q.elements, -1
        )  # w,x,y,z -> x,y,z,w (which pybullet expects)
        p.resetBasePositionAndOrientation(
            bodyUniqueId=body.bid,
            posObj=position,
            ornObj=rot_q,
            physicsClientId=self._pid,
        )
    ...
    def get_contacts(
        ...
        obj_id_a=None,
        obj_id_b=None,
        only_closest_contact_per_pair=True,
    ...
    def ray_test(self, from_pos, to_pos):
        hit = p.rayTest(
            rayFromPosition=from_pos, rayToPosition=to_pos, physicsClientId=self._pid
        )
...
if __name__ == "__main__":
    ...
    h = House(
        id_="0f7759ed-ccf3-4115-bd00-cf4d8165e6d3", include_arch_information=False
    )
    # h = House(index=5)
    sim.add_house(
        h, no_walls=False, no_ceil=True, use_separate_walls=False, static=False
    )
    sim.run()
