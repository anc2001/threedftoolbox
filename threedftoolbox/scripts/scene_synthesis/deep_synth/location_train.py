...
parser.add_argument(
    "--last-epoch", type=int, default=-1, metavar="N"
)  # If positive, use saved epoch
...
parser.add_argument(
    "--use-count", action="store_true", default=False
)  # Use category count if true
parser.add_argument(
    "--no-penalty", action="store_true", default=False
)  # If True, L_Global is not used
parser.add_argument(
    "--progressive-p", action="store_true", default=False
)  # If True, start with lower p_auxiliary and gradually increase
...
LOG("Building model...")
model = resnet101(
    num_classes=num_categories + 3, num_input_channels=num_input_channels, use_fc=False
)
if args.use_count:
...
def train():
    ...
    for batch_idx, (data, t, existing, penalty) in enumerate(train_loader):
        ...
        if num_seen % 10000 == 0:
            ...
            if current_epoch % 5 == 0:
                ...
                torch.save(
                    fc.state_dict(), f"{save_dir}/location_fc_{current_epoch}.pt"
                )
                torch.save(
...
def validate():
    ...
    total_loss = 0
    total_accuracy = 0
    for batch_idx, (data, t, existing, penalty) in enumerate(validation_loader):
