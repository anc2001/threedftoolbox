...
class ObjectCollection:
    ...
    def update(self, o, xform=None, update_sim=False):
        ...
        o.obb = OBB.from_local2world_transform(
            np.matmul(o.xform.as_mat4(), o.model2world)
        )
        ...
        if update_sim:
            self._sim.set_state(
                obj_id=o.id, position=o.xform.translation, rotation_q=o.xform.rotation
            )
    ...
    def as_house(self):
        room_nodes = [
            dict(n.__dict__) for n in self._objects.values() if n.type != "Room"
        ]
        ...
        room = {
            ...
            "originalId": (
                ...
                if hasattr(self.room, "originalId")
                else self.room.id
            ),
            "type": "Room",
            "valid": 1,
        ...
        for o_r in ref_objects:
            ...
            for o_i in self._objects.values():
                ...
                if o_i.category in ignore_categories:
                    continue
                out[(o_i.id, o_r.id)] = self.object_frames_to_relative_observation(
                    o_i.frame, o_r.frame, room_id
                )
        return out
    ...
    def get_collisions(self, include_collision_with_static=True, obj_id_a=None):
        ...
        return self._sim.get_contacts(
            obj_id_a=obj_id_a,
            include_collision_with_static=include_collision_with_static,
        )

    def get_observation_key(self, observation):
        room_node = self._objects[observation.room_id]
        room_types = (
            "-".join(room_node.roomTypes) if hasattr(room_node, "roomTypes") else ""
        )
        ...
        key = ObservationCategory(
            room_types=room_types, obj_category=obj_cat, ref_obj_category=ref_cat
        )
        return key
    ...
    def object_frames_to_relative_observation(self, frame, ref_frame, room_id):
        ...
        rel_up = ref_frame["obb"].transform_direction(
            frame["obb"].rotation_matrix[:3, 1]
        )
        rel_front = ref_frame["obb"].transform_direction(
            -frame["obb"].rotation_matrix[:3, 2]
        )  # note: +z = back
        cp = self._sim.get_closest_point(
            obj_id_a=frame["obj_id"], obj_id_b=ref_frame["obj_id"]
        )
        ...
        out = RelativeObservation(
            room_id=room_id,
            obj_id=frame["obj_id"],
            ref_id=ref_frame["obj_id"],
            ref_dims=ref_dims,
...
class RelativeObservation(
    ...
    def to_str_list(self):
        return [
            ...
            self.obj_id,
            self.ref_id,
            nparr2str_compact(self.ref_dims),
            nparr2str_compact(self.centroid),
            nparr2str_compact(self.min),
            nparr2str_compact(self.max),
            nparr2str_compact(self.closest),
            nparr2str_compact(self.front),
            nparr2str_compact(self.up),
        ]
    ...
    def fromstring(cls, s):
        return RelativeObservation(
            ...
            ref_id=s["ref_id"],
            ref_dims=s["ref_dims"],
            centroid=str2nparr(s["centroid"]),
            min=str2nparr(s["min"]),
            max=str2nparr(s["max"]),
            closest=str2nparr(s["closest"]),
            front=str2nparr(s["front"]),
            up=str2nparr(s["up"]),
        )
    ...
    def parameterize(self, scheme):
        if scheme == "dist_angles":
            ...
            raise RuntimeError(
                f"Unknown RelativeObservation parameterization scheme {scheme}"
            )
...
ObservationCategory = namedtuple(
    "ObservationCategory", ["room_types", "obj_category", "ref_obj_category"]
)
...
class RelativeObservationsDatabase:
    ...
    def __init__(self, name, priors_dir, verbose=False, categorization_type="final"):
        ...
        self._observations = (
            {}
        )  # house_id -> {(obj_id,ref_obj_id): RelativeObservation}
    ...
    def save_observations_by_house(self, prefix="", save_frames=False):
        ...
        if save_frames:
            for house_id, house_frames in self._semantic_frames.items():
                ...
                f = csv.writer(open(csv_file, "w"))
                f.writerow(
                    ["obj_id", "local2world", "world_aabb_min", "world_aabb_max"]
                )
                for frame in house_frames.values():
                    local2world_flat = nparr2str_compact(
                        np.squeeze(np.asarray(frame["obb"].local2world.flatten()))
                    )
                    f.writerow(
                        [
                            frame["obj_id"],
                            local2world_flat,
                            nparr2str_compact(frame["aabb"]["min"]),
                            nparr2str_compact(frame["aabb"]["max"]),
                        ]
                    )
    ...
    def load_observations(self, filenames, house_dir, load_frames=False):
        ...
        for i, (obs_id, house_id) in enumerate(counts_house_ids):
            ...
            for row in csv.DictReader(
                open(
                    os.path.join(
                        self._priors_dir, obs_id + "_" + house_id + ".relpos.csv"
                    )
                )
            ):
                if len(row) == 0:
            ...
            if load_frames:
                ...
                for row in csv.DictReader(
                    open(
                        os.path.join(
                            self._priors_dir, obs_id + "_" + house_id + ".semframes.csv"
                        )
                    )
                ):
        ...
        for i, (obs_id, house_id) in enumerate(counts_house_ids):
            ...
            house = House(
                file_dir=os.path.join(house_dir, obs_id + ".json"),
    ...
    def _get_observations_from_house(self, house):
        ...
        relative_observations = (
            {}
        )  # (node_i.id,node_j.id) -> observation of node_i relative to node_j
        ...
        for room in house.rooms:
            ...
            room_relative_observations = self._objects.get_relative_observations(
                room.id, filter_ref_obj=None, ignore_categories=list()
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--task", type=str, required=True, help="<Required> task [collect|save_pkl]"
    )
    parser.add_argument(
        "--input", type=str, help="house json or directory with house json files"
    )
    parser.add_argument(
        "--priors_dir", type=str, required=True, help="priors data directory"
    )
    ...
    if args.task == "collect":
        for f in house_files:
            ...
            priors_file = os.path.join(
                args.priors_dir, prefix + "_" + house.id + ".relpos.csv"
            )
    ...
    if args.task == "save_pkl":
        ...
        relpos_files = list(
            map(lambda p: os.path.basename(p).split(".")[0], relpos_files)
        )
        print(relpos_files)
