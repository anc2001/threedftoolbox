...
class ArrangementPriors:
    ...
    def __init__(
        ...
        self._num_priors = 0.0
        self._num_observations = 0.0
        self._w_dist = w_dist
    ...
    def log_prob(self, pairwise_observations_by_key):
        ...
        for key in pairwise_observations_by_key:
            ...
            records = map(
                lambda o: o.parameterize(scheme="offsets_angles"), observations
            )
...
class ArrangementGreedySampler:
    ...
    def init(self, house, only_architecture=True, room_id=None):
        ...
        self._objects.init_from_room(
            house, room_id, only_architecture=only_architecture
        )
    ...
    def get_candidate_transform(self, node, max_iterations=100):
        ...
        while True:
            ...
            ray_from = [p[0], zmin - 0.5, p[2]]
            ray_to = [p[0], zmin + 0.5, p[2]]
            ...
            if intersection.id == self.room_id + "f" or num_checks > max_iterations:
                break
        xform = Transform()
        xform.set_translation([p[0], zmin + 0.1, p[2]])
    ...
    def sample_placement(
        ...
        for i in range(max_attempts_per_sample * n_samples):
            ...
            if collision_threshold > 0:
                if (
                    min(collisions.values(), key=lambda c: c.distance).distance
                    < -collision_threshold
                ):
                    continue
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--input", type=str, required=True, help="input scene to arrange"
    )
    parser.add_argument(
        "--output_dir",
        type=str,
        required=True,
        help="output directory to save arranged house states",
    )
    parser.add_argument(
        "--priors_dir", type=str, required=True, help="base directory to load priors"
    )
    parser.add_argument(
        "--only_final_state", action="store_true", help="save only final"
    )
    parser.add_argument(
        "--sim_mode",
        dest="sim_mode",
        default="direct",
        help="sim server [gui|direct|shared_memory]",
    )
    parser.add_argument(
        "--restart_sim_every_round",
        action="store_true",
        help="restart sim after every object placed",
    )
    parser.add_argument(
        "--max_objects",
        type=int,
    ...
    parser.add_argument(
        ...
        type=int,
        default=100,
        help="placement samples per object",
    )
    parser.add_argument(
        "--ignore_doors_windows",
        action="store_true",
        help="ignore door and window priors",
    )
    parser.add_argument(
        "--collision_threshold",
        type=float,
        default=0,
        help="how much collision penetration to allow",
    )
    ...
    if args.task == "arrange":
        ...
        ap = ArrangementPriors(
            priors_file=filename,
            w_dist=0.0,
            w_clearance=1.0,
            w_closest=1.0,
            w_orientation=1.0,
        ...
        for i, n in enumerate(placeables):
            ...
            if (
                args.sim_mode == "gui" and i < 2
            ):  # center view on first couple of placements if gui_mode
                ags.objects.simulator.set_gui_rendering(enabled=True)
            houses_log = None if args.only_final_state else houses_states
            ags.sample_placement(
                node=n,
                n_samples=args.num_samples_per_object,
                houses_log=houses_log,
                ignore_categories=(
                    ["window", "door"] if args.ignore_doors_windows else []
                ),
            )
